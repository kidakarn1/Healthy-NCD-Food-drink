...
<div class="panel">
            ...
            <div class=" panel panel-primary">
                ...
                <div class="panel-heading"><center><h3>อาหารสำหรับคุณ <?php echo $_SESSION[fname] . " " . $_SESSION[lname]; ?></h3></center>
                    <div class="row">
                ...
                <div class="three panel-body">
                    ...
                    $sql_dis = "select * from member where mem_id='{$_SESSION['mem_id']}'";
                    ...
                    // exit();
                    $k = 0;
                    $sql = "select * from member,food where member.mem_id='{$_SESSION['mem_id']}' and
                    food.dis_id and food.cat_id='1'in ('$new_str')";
...
// echo $new_str;
                    ...
                    for ($i = 1; $i <= 7; $i++) {
                        ...
                        $dayweek = date("l", strtotime("+$datet days", strtotime($date1))); //วันภาษาอังกฤษอีกอย่างหนึ่ง
                        echo $datedate1 = date("d-m-Y", strtotime("+$datet day", strtotime($date1)));
                        $datedate = date("Y-m-d", strtotime("+$datet day", strtotime($date1))); //การบวกวันแบบตัวแปร

                        $datet++;

                        echo "<br>" . $day_mon . $data[$dayweek]; //แสดงวัน
                        //  if ($daypangubun==$){
                        //     echo $day_mon.$data[0];
                        //  }
                        ...
                        for ($j = 0; $j <= 2; $j++) {
                                    ...
                                    <div class="tableproduct thumbnail ">
                                        ...
                                        <center>
                                            <div class=" tableproduct1 caption">
                                                ...
                                                <h5>ชื่อ <?php echo $row[$k]["food_name"] . " " . $row[$k]["Description"]; ?><p></p>
                                                    <form action='inorder.php' method='post' name='form2' id='form2'>
                                                        ...
                                                        <button type="button" class="btn btn-success btn-sm " data-toggle="modal" data-target="#myModal<?php echo $row[$k]['food_id'] ?>">
                            ...
                            $bol = false;
                            $jd = cal_to_jd(CAL_GREGORIAN, $day[1], $ee = $day[2] + $i, $day[0]); //2011-01-29
                            $daypangubun = (jddayofweek($jd, 1));
                            // print_r($daypangubun);
                    ...
                    if ($bol) {
                        $sql_food_status = "insert into food_status values(
									 '',
									 '{$_SESSION['mem_id']}',
									 '1'
									 )";
                        mysqli_query($conn, $sql_food_status);
                    }
                    echo "<p>" . "</p>";
                    ?>         
